...
- Page object didaftarkan di `fixtures/page.fixture.ts`; test mengaksesnya lewat dependency injection (TIDAK di-`new` langsung, kecuali popup window).
- **Auth**: proyek memakai `auth.setup.ts` (project `setup`) yang menyimpan `storageState` ke `playwright/.auth/user.json`; semua browser project (`firefox` | `chrome` | `safari`) bergantung padanya. Login UI per-test disarankan memakai `loginAsTestUser()` dari `utils/AuthFlow.ts` (smart — cek storageState dulu), bukan `loginAs()` tiap test.
...
- `components/` masih kosong — jangan minta build agent membuat komponen tanpa kebutuhan jelas.
...
## Checklist Delegasi (agar prompt lengkap)

Saat mendelegasikan ke Build Agent, pastikan prompt mencakup:
- File/area yang terlibat (page object, spec, constant, data, fixture).
- Konvensi yang relevan (import `.js`, DI via fixture, no hardcode, idempotensi).
- Instruksi menjalankan `npx tsc --noEmit` setelah selesai.
